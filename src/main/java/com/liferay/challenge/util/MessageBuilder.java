...
public class MessageBuilder {
	...
	public static String build(
		String organizationName, ArrayList<Integer> divisibleValues) {

		if (organizationName == null || organizationName.equals("") ||
			divisibleValues == null) {
			return "";
		}
		int size = divisibleValues.size();
		StringBuffer messageBuffer = new StringBuffer(organizationName);
		if (size > 0) {
			messageBuffer.append(", divisible en grupos de ").append(
				divisibleValues.remove(0));
			size--;
